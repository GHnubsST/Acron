...
private:
    ...
    {
        const int optval = 1;
        if (setsockopt(mfd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &optval, sizeof(optval)) == -1)
        {
            throw std::runtime_error(std::string(strerror(errno)));
        }
    }
    ...
    void acorn_bindSocket(const int &mfd, const std::pair<std::string, uint16_t> &addrPort)
    {
        ...
        {
            address = "::ffff:" + address.substr(0, address.find('/'));
        }
    ...
    void acorn_listener()
    {
        for (const auto &addrPort : addressPort)
        {
            ...
            try
            {
                ...
                std::cout << "Socket listening on " << addrPort.first << ":" << addrPort.second << std::endl;
            }
